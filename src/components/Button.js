...
export default function ButtonDefault({ text, url, variant }) {
    let button = 'min-w-[140px] max-sm:min-w-[130px] min-h-[35px] max-sm:min-h-[40px] h-fit w-fit px-10 max-sm:px-3 max-sm:text-sm text-xl text-white text-center rounded-[32px] transition-all mx-auto hover:scale-[90%] max-sm:hover:scale=[101%] focus:outline-0 flex justify-center'
    if (variant !== null){
        switch (variant) {
            case 1:
